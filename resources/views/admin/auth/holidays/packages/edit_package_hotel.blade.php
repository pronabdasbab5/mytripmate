                    ...
                    <div class="col-md-8 col-sm-8 col-xs-12">
                        <div class="pacakge-hotel-div col-md-7 col-xs-12  form-control form-text-element">
                            @if(count($phrelationData) > 0)
                                @foreach($phrelationData as $value)

                                    <input type="hidden" name="hotel_hidden_id[]" value="{{ $value->id }}" checked>

                                    @if($value->status == 1)
                                        <input type="checkbox" name="hotel_id[]" value="{{ $value->id }}" checked>&nbsp;&nbsp;&nbsp;<b>{{ ($value->hotelType == 1)? "Budget": "Delux" }}, <a href="{{ route('admin.p_hotel_edit_form', ['pHotelId' => $value->hotelId]) }}" title="Click Me for Details" target="_blank">{{ $value->hotelName }}</a> , Rs. {{ $value->price }}</b><br>
                                    @else
                                        <input type="checkbox" name="hotel_id[]" value="{{ $value->id }}">&nbsp;&nbsp;&nbsp;<b>{{ ($value->hotelType == 1)? "Budget": "Delux" }}, <a href="{{ route('admin.p_hotel_edit_form', ['pHotelId' => $value->hotelId]) }}" title="Click Me for Details" target="_blank">{{ $value->hotelName }}</a> , Rs. {{ $value->price }}</b><br>
                                    @endif
                                @endforeach
                            @endif
                        </div>
